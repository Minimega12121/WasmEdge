...
Expect<uint32_t> WasiFdPread::body(const Runtime::CallingFrame &Frame,
  ...
  if (IOVsPtr % alignof(__wasi_iovec_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
...
Expect<uint32_t> WasiFdReadDir::body(const Runtime::CallingFrame &Frame,
                                     ...
                                     uint32_t /* Out */ NReadPtr) {
  // Alignment checks
  // no alignment requirement for BufPtr
  if (NReadPtr % alignof(__wasi_size_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
...
Expect<uint32_t> WasiPathLink::body(const Runtime::CallingFrame &Frame,
                                    ...
                                    uint32_t NewPathLen) {
  // No alignment checks needed for path pointers (char data has 1-byte
  // alignment)
...
Expect<uint32_t> WasiPathReadLink::body(const Runtime::CallingFrame &Frame,
  ...
  // no alignment checks for PathPtr and BufPtr, as char has alignment of 1
  // byte.
...
Expect<uint32_t> WasiSockRecvV1::body(const Runtime::CallingFrame &Frame,
  ...
  // Alignment checks
  if (RiDataPtr % alignof(__wasi_iovec_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }

  if (RoDataLenPtr % alignof(__wasi_size_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }

  if (RoFlagsPtr % alignof(__wasi_roflags_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
...
Expect<uint32_t> WasiSockSendV1::body(const Runtime::CallingFrame &Frame,
  ...
  // Alignment checks
  if (SiDataPtr % alignof(__wasi_ciovec_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }

  if (SoDataLenPtr % alignof(__wasi_size_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
...
Expect<uint32_t> WasiSockSetOpt::body(const Runtime::CallingFrame &Frame,
                                      ...
                                      uint32_t FlagSize) {
  // Alignment checks
  // no alignment checks for FlagPtr, as uint8_t has alignment of 1 byte
...
Expect<uint32_t> WasiSockGetAddrinfo::body(
  ...
  // Alignment checks
  // no alignment checks for NodePtr and ServicePtr, as uint8_t has alignment of
  // 1 byte

  if (HintsPtr % alignof(__wasi_addrinfo_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
  if (ResPtr % alignof(uint8_t_ptr) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
  if (ResLengthPtr % alignof(__wasi_size_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
...
WasiSockGetLocalAddrV1::body(const Runtime::CallingFrame &Frame, int32_t Fd,
  ...
  if (AddressTypePtr % alignof(uint32_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }

  if (PortPtr % alignof(uint32_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
...
Expect<uint32_t> WasiSockGetOpt::body(const Runtime::CallingFrame &Frame,
  ...
  if (FlagSizePtr % alignof(uint32_t) != 0) {
    return __WASI_ERRNO_ADDRNOTAVAIL;
  }
